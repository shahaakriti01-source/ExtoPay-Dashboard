...
export function calculateMerchantIncentive(merchantName, onboardingEvents, payments, config) {
  ...
  // 4. Fixed transaction-processing incentive
  const fixedIncentive = cappedCount * config.fixedRate;

  // 5. Variable transaction-processing incentive (only on transactions sent BY the merchant,
  //    capped per-transaction, up to the overall txn cap)
  const sentQualifying = qualifying.filter(p => p.sender === merchantName);
  const variableIncentive = sentQualifying.slice(0, config.txnCap).reduce((sum, p) => {
