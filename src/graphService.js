...
// Builds an Account ID -> resolved Name lookup from the "Mapping Person to ID"
// sheet. This is the authoritative source for names — the Onboarding sheet's
// own "Agent name" column is frequently "#N/A" (e.g. whenever Exto or a field
// coordinator is the Authorize), so we resolve names ourselves rather than
// relying on that column.
function buildAccountIdToNameMap(mappingRows) {
  const map = new Map();
  mappingRows.slice(4).forEach(row => {
    const accountId = row[9] ? String(row[9]).trim() : null;
    const name = row[16] ? String(row[16]).trim() : null;
...
export async function fetchLivePilotData() {
  ...
  const [mappingRows, onboardingRows, paymentsRows] = await Promise.all([
    getSheetValues(driveId, itemId, "Mapping Person to ID", token),
  ...
  onboardingRows.slice(4).forEach(row => {
    ...
    const authorizeId = row[5] ? String(row[5]).trim() : "";
